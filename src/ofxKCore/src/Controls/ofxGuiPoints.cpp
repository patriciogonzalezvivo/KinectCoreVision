...
void ofxGuiPoints::init(int id, string name, int x, int y, int width, int height, ofPoint min, ofPoint max, ofPoint value, int display, int steps)
{
	...
	mList.points.push_back(ofPoint(mMinVal.x, mMinVal.y + mValDlt.y * 0.5)); 
	mList.points.push_back(ofPoint(mMaxVal.x, mMinVal.y + mValDlt.y * 0.5));
...
void ofxGuiPoints::setValue(ofPoint value)
{
	mValue = value;
}
...
void ofxGuiPoints::setRange(ofPoint min, ofPoint max) 
{
	mMinVal	= min;
	mMaxVal	= max;
...
{
	bool		handled	= false;
	ofPoint	value	= mValue;
	
	if(id == mParamId)
	{
		if(task == kofxGui_Set_Float)
			setValue(ofPoint(mMinVal.x + CLAMP(*(float*)data, 0.0, 1.0) * mValDlt.x, mValue.y));
		else if(task == kofxGui_Set_Point)
			setValue(*(ofPoint*)data);
	...
	if(value!= mValue)
	{
		...
		mGlobals->mListener->handleGui(mParamId, kofxGui_Set_Point, &mOutVal, sizeof(ofPoint));
	}
...
void ofxGuiPoints::draw()
{
	glPushMatrix();
		...
		ofPoint	p	= fractionToLocal(valueToFraction(mValue));
		...
		ofBeginShape();
			...
			{
				ofPoint p = fractionToLocal(valueToFraction(mList.points.at(i)));				
				ofVertex(p.x, p.y);
			}
		ofEndShape(false);
...
bool ofxGuiPoints::mouseDragged(int x, int y, int button)
{
	...
	{
		ofPoint	value	= mValue;
		ofPoint	point	= fractionToValue(mouseToFraction(mouseToLocal(x, y)));
...
{
	ofPoint point = mouseToLocal(x, y);

	if(isPointInsideMe(point))
	{
		ofPoint	inside	= fractionToValue(mouseToFraction(point));
...
void ofxGuiPoints::buildFromXml()
{
	...
	if(numberOfTags > 0)
	{
		...
		for(int i = 0; i < numberOfTags; i++)
		{
			...
			mList.points.push_back(ofPoint(x, y)); 
...
ofPoint ofxGuiPoints::valueToFraction(ofPoint value)
{
	return ofPoint((value.x - mMinVal.x) / mValDlt.x, (value.y - mMinVal.y) / mValDlt.y);
}

//	----------------------------------------------------------------------------------------------------

ofPoint ofxGuiPoints::fractionToValue(ofPoint fraction)
{
	return ofPoint((mValDlt.x * fraction.x) + mMinVal.x, (mValDlt.y * fraction.y) + mMinVal.y);
}

//	----------------------------------------------------------------------------------------------------

bool ofxGuiPoints::isPointWithinDistance(ofPoint position)
{
	mList.activePoint = -1;
	
	for(int i = 0; i < mList.points.size(); i++)
	{
		ofPoint	point	= mList.points.at(i);
