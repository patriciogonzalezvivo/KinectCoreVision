...
void ofxGuiScope::init(int id, string name, int x, int y, int width, int height, int length, ofPoint value, int display)
{
...
void ofxGuiScope::setValue(ofPoint value)
{
	//	todo:	steps
...
bool ofxGuiScope::update(int id, int task, void* data, int length)
{
	...
	if(id == mParamId)
	{
		if(task == kofxGui_Set_Float)
			setValue(ofPoint(CLAMP(*(float*)data, 0.0, 1.0) * mBufferLength, mValue.y));
...
ofPoint ofxGuiScope::valueToFraction(ofPoint value)
{
	return ofPoint(value.x / mBufferLength, value.y);
}

//	----------------------------------------------------------------------------------------------------

ofPoint ofxGuiScope::fractionToValue(ofPoint fraction)
{
	return ofPoint(fraction.x * mBufferLength, fraction.y);
}
