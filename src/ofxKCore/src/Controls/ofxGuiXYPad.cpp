...
void ofxGuiXYPad::init(int id, string name, int x, int y, int width, int height, ofPoint min, ofPoint max, ofPoint value, int display, int steps)
{
...
void ofxGuiXYPad::setValue(ofPoint value)
{
	mValue = value;
}
...
void ofxGuiXYPad::setRange(ofPoint min, ofPoint max) 
{
	mMinVal	= min;
	mMaxVal	= max;
...
bool ofxGuiXYPad::update(int id, int task, void* data, int length)
{
	...
	if(id == mParamId && length == sizeof(ofPoint))
	{
...
bool ofxGuiXYPad::mouseDragged(int x, int y, int button)
{
	if(mMouseIsDown)
	{
		ofPoint value = fractionToValue(mouseToFraction(mouseToLocal(x, y)));

		if(value != mValue)
		{
			setValue(value);
			mGlobals->mListener->handleGui(mParamId, kofxGui_Set_Point, &mValue, sizeof(ofPoint));
		}
	}
...
ofPoint ofxGuiXYPad::valueToFraction(ofPoint value)
{
...
ofPoint ofxGuiXYPad::fractionToValue(ofPoint fraction)
{
